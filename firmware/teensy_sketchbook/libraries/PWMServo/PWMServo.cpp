...
void PWMServo::writef(float angleArg)
{
	//Serial.printf("write, pin=%d, angle=%d\n", pin, angleArg);
	if (pin >= NUM_DIGITAL_PINS) return;
	if (angleArg < 0.0f) angleArg = 0.0f;
	if (angleArg > 180.0f) angleArg = 180.0f;
	anglef = angleArg;
	angle = lroundf(anglef);
	float range = max16 - min16;
	float us = ((range * angleArg) * 22.75537109375f) + (min16 * 4096.0f);
	float duty = us / 1250.165126676602086438152011922503725782414307004470938897f;
	//float usec = (float)((max16 - min16)<<4) * ((float)angle / 180.0f) + (float)(min16<<4);
	//uint32_t duty = (int)(usec / 20000.0f * 4096.0f);
	//Serial.printf("angle=%d, usec=%.2f, us=%.2f, duty=%d, min=%d, max=%d\n",
		//angle, usec, (float)us / 256.0f, duty, min16<<4, max16<<4);
