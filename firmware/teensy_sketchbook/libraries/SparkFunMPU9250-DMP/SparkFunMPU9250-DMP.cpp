...
bool MPU9250_DMP::asyncStateMachine(void)
{
	inv_error_t err;

	switch (this->asyncStateMachine)
	{
		case DMPASYNCSTATE_INIT:
		case DMPASYNCSTATE_FIFOH_START:
			mpu_read_reg_asyncStart(MPU9250_FIFO_COUNTH);
			this->asyncStateMachine = DMPASYNCSTATE_FIFOCOUNTH_WAIT;
			return false;
		case DMPASYNCSTATE_FIFOH_WAIT:
			if (mpu_spi_read_dmaIsBusy() == false) {
				this->asyncStateMachine = DMPASYNCSTATE_FIFOCOUNTL_START;
				mpu_read_reg_asyncEnd(&(this->asyncCountH));
			}
			//return false;
		case DMPASYNCSTATE_FIFOCOUNTL_START:
			mpu_read_reg_asyncStart(MPU9250_FIFO_COUNTL);
			this->asyncStateMachine = DMPASYNCSTATE_FIFOCOUNTL_WAIT;
			return false;
		case DMPASYNCSTATE_FIFOCOUNTL_WAIT:
			if (mpu_spi_read_dmaIsBusy() == false) {
				mpu_read_reg_asyncEnd(&(this->asyncCountL));
				uint32_t c = this->asyncCountH;
				c <<= 8;
				c += this->asyncCountL;
				if (c > 0) {
					this->asyncStateMachine = DMPASYNCSTATE_FIFORW_START;
				}
				else {
					this->asyncStateMachine = DMPASYNCSTATE_FIFOH_START;
					return false;
				}
			}
			//return false;
		case DMPASYNCSTATE_FIFORW_START:
			dmp_read_fifo_asyncStart();
			this->asyncStateMachine = DMPASYNCSTATE_FIFORW_WAIT;
			return false;
		case DMPASYNCSTATE_FIFORW_WAIT:
			if (mpu_spi_read_dmaIsBusy() == false) {
				this->asyncStateMachine = DMPASYNCSTATE_FIFOCOUNTH_START;
				err = dmpUpdateFifo_asyncEnd();
				if (err == INV_SUCCESS) {
					return true;
				}
			}
			return false;
		default:
			this->asyncStateMachine = DMPASYNCSTATE_INIT;
	}
	return false;
}
