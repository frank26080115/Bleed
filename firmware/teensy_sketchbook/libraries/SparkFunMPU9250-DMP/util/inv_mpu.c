...
#define spi_write(b, c, d)    mpu_spi_write(b, c, d)
#define spi_read(b, c, d)     mpu_spi_read(b, c, d)

#define MPU_USE_SPI

#ifdef MPU_USE_I2C
#define mpu_reg_write(b, c, d) mpu_i2c_write(b, c, d)
#define mpu_reg_read(b, c, d)  mpu_i2c_read(b, c, d)
#elif defined(MPU_USE_SPI)
#define mpu_reg_write(b, c, d) spi_write(b, c, d)
#define mpu_reg_read(b, c, d)  spi_read(b, c, d)
#else
#error
#endif
...
int set_int_enable(unsigned char enable)
{
	...
	if (st.chip_cfg.dmp_on) {
		if (enable)
			...
			tmp = 0x00;
		if (mpu_reg_write(st.reg->int_enable, 1, &tmp))
			return -1;
...
int mpu_init(struct int_param_s *int_param)
{
	...
	/* Reset device. */
	data[0] = BIT_RESET;
	if (mpu_reg_write(st.reg->pwr_mgmt_1, 1, data))
		return -1;
	delay_ms(100);
	...
	mpu_reg_write(st.reg->pwr_mgmt_1, 1, data);
   ...
   st.chip_cfg.accel_half = 0;
	...
	if (mpu_reg_write(st.reg->accel_cfg2, 1, data))
		return -1;
#endif
...
int mpu_lp_accel_mode(unsigned short rate)
{
	...
	if (!rate) {
		...
		tmp[0] = 0;
		tmp[1] = BIT_STBY_XYZG;
		if (mpu_reg_write(st.reg->pwr_mgmt_1, 2, tmp))
			return -1;
		st.chip_cfg.lp_accel_mode = 0;
		return 0;
	}
	...
	if (rate == 1)
		...
		tmp[0] = INV_LPA_640HZ;
	if (mpu_reg_write(st.reg->lp_accel_odr, 1, tmp))
		return -1;
	tmp[0] = BIT_LPA_CYCLE;
	if (mpu_reg_write(st.reg->pwr_mgmt_1, 1, tmp))
		return -1;
#endif
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
	...
	if (mpu_reg_read(st.reg->raw_gyro, 6, tmp))
		return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
	...
	if (mpu_reg_read(st.reg->raw_accel, 6, tmp))
		return -1;
	data[0] = (tmp[0] << 8) | tmp[1];
	data[1] = (tmp[2] << 8) | tmp[3];
	data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
	...
	if (mpu_reg_read(st.reg->temp, 2, tmp))
		return -1;
	raw = (tmp[0] << 8) | tmp[1];
	if (timestamp)
		get_ms(timestamp);
...
int mpu_read_6500_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (mpu_reg_read(0x77, 2, &data[0]))
		return -1;
	if (mpu_reg_read(0x7A, 2, &data[2]))
		return -1;
	if (mpu_reg_read(0x7D, 2, &data[4]))
		return -1;
...
int mpu_read_6050_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (mpu_reg_read(0x06, 2, &data[0]))
		return -1;
	if (mpu_reg_read(0x08, 2, &data[2]))
		return -1;
	if (mpu_reg_read(0x0A, 2, &data[4]))
		return -1;
	accel_bias[0] = ((long)data[0]<<8) | data[1];
	accel_bias[1] = ((long)data[2]<<8) | data[3];
	accel_bias[2] = ((long)data[4]<<8) | data[5];
...
int mpu_read_6500_gyro_bias(long *gyro_bias) {
	unsigned char data[6];
	if (mpu_reg_read(0x13, 2, &data[0]))
		return -1;
	if (mpu_reg_read(0x15, 2, &data[2]))
		return -1;
	if (mpu_reg_read(0x17, 2, &data[4]))
		return -1;
...
int mpu_set_gyro_bias_reg(long *gyro_bias)
{
	...
	if (mpu_reg_write(0x13, 2, &data[0]))
		return -1;
	if (mpu_reg_write(0x15, 2, &data[2]))
		return -1;
	if (mpu_reg_write(0x17, 2, &data[4]))
		return -1;
	return 0;
}
...
int mpu_set_accel_bias_6050_reg(const long *accel_bias) {
	...
	if (mpu_reg_write(0x06, 2, &data[0]))
		return -1;
	if (mpu_reg_write(0x08, 2, &data[2]))
		return -1;
	if (mpu_reg_write(0x0A, 2, &data[4]))
		return -1;
...
int mpu_set_accel_bias_6500_reg(const long *accel_bias) {
	...
	if (mpu_reg_write(0x77, 2, &data[0]))
		return -1;
	if (mpu_reg_write(0x7A, 2, &data[2]))
		return -1;
	if (mpu_reg_write(0x7D, 2, &data[4]))
		return -1;
...
int mpu_reset_fifo(void)
{
	...
	data = 0;
	if (mpu_reg_write(st.reg->int_enable, 1, &data))
		return -1;
	if (mpu_reg_write(st.reg->fifo_en, 1, &data))
		return -1;
	...
	if (mpu_reg_write(st.reg->user_ctrl, 1, &data))
		return -1;
	...
	if (st.chip_cfg.dmp_on) {
		...
		if (mpu_reg_write(st.reg->int_enable, 1, &data))
			return -1;
		if (mpu_reg_write(st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
			return -1;
	}
	return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
	...
	if (st.chip_cfg.gyro_fsr == (data >> 3))
		return 0;
	if (mpu_reg_write(st.reg->gyro_cfg, 1, &data))
		return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
	...
	if (st.chip_cfg.accel_fsr == (data >> 3))
		return 0;
	if (mpu_reg_write(st.reg->accel_cfg, 1, &data))
		return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
	...
	if (st.chip_cfg.lpf == data)
		return 0;
	if (mpu_reg_write(st.reg->lpf, 1, &data))
		return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
	...
	if (st.chip_cfg.dmp_on)
		...
		data = 1000 / rate - 1;
		if (mpu_reg_write(st.reg->rate_div, 1, &data))
			return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
	...
	if (mpu_reg_write(st.reg->s4_ctrl, 1, &div))
		return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
	...
	if (!(sensors & INV_XYZ_ACCEL))
		data |= BIT_STBY_XYZA;
	if (mpu_reg_write(st.reg->pwr_mgmt_2, 1, &data)) {
		st.chip_cfg.sensors = 0;
		return -1;
	}
...
#else
	#ifdef MPU_USE_SPI
	user_ctrl |= BIT_I2C_IF_DIS;
	#endif
	if (mpu_reg_read(st.reg->user_ctrl, 1, &user_ctrl))
		return -1;
	...
	if (st.chip_cfg.dmp_on)
		...
		user_ctrl &= ~BIT_DMP_EN;
	if (mpu_reg_write(st.reg->s1_do, 1, &data))
		return -1;
	/* Enable/disable I2C master mode. */
	#ifdef MPU_USE_SPI
	user_ctrl |= BIT_I2C_IF_DIS;
	#endif
	if (mpu_reg_write(st.reg->user_ctrl, 1, &user_ctrl))
		return -1;
#endif
#endif
...
int mpu_get_int_status(short *status)
{
	...
	if (!st.chip_cfg.sensors)
		return -1;
	if (mpu_reg_read(st.reg->dmp_int_status, 2, tmp))
		return -1;
		...
		unsigned char *sensors, unsigned char *more)
{
	...
	if (mpu_reg_read(st.reg->fifo_count_h, 2, data))
		return -1;
...
//    log_i("FIFO count: %hd\n", fifo_count);
	if (fifo_count > (st.hw->max_fifo >> 1)) {
		...
		if (mpu_reg_read(st.reg->int_status, 1, data))
			return -1;
	...
	if (mpu_reg_read(st.reg->fifo_r_w, packet_size, data))
		return -1;
	...
	unsigned char *more)
{
	...
	if (mpu_reg_read(st.reg->fifo_count_h, 2, tmp))
		return -1;
	...
	if (fifo_count > (st.hw->max_fifo >> 1)) {
		/* FIFO is 50% full, better check overflow bit. */
		if (mpu_reg_read(st.reg->int_status, 1, tmp))
			return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
	...
	if (bypass_on) {
		if (mpu_reg_read(st.reg->user_ctrl, 1, &tmp))
			return -1;
		tmp &= ~BIT_AUX_IF_EN;
		if (mpu_reg_write(st.reg->user_ctrl, 1, &tmp))
			return -1;
		delay_ms(3);
		tmp = BIT_BYPASS_EN;
		...
		if (mpu_reg_write(st.reg->int_pin_cfg, 1, &tmp))
			return -1;
	} else {
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
	...
	data[0] = 0x01;
	data[1] = 0;
	if (mpu_reg_write(st.reg->pwr_mgmt_1, 2, data))
		return -1;
	delay_ms(200);
	data[0] = 0;
	if (mpu_reg_write(st.reg->int_enable, 1, data))
	...
	if (mpu_reg_write(st.reg->pwr_mgmt_1, 1, data))
		return -1;
	if (mpu_reg_write(st.reg->i2c_mst, 1, data))
		return -1;
	#ifdef MPU_USE_SPI
	data[0] |= BIT_I2C_IF_DIS;
	#endif
	if (mpu_reg_write(st.reg->user_ctrl, 1, data))
		return -1;
	...
	for (ii = 0; ii < packet_count; ii++) {
		...
		if (mpu_reg_read(st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
			return -1;
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	if (mpu_reg_read(REG_6500_XA_ST_DATA, 3, regs)) {
		if(debug)
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	if (mpu_reg_read(REG_6500_XG_ST_DATA, 3, regs)) {
		if(debug)
			log_i("Reading OTP Register Error.\n");
		return 0x07;
	}
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
	...
	while (s < test.packet_thresh) {
		...
		if (mpu_reg_read(st.reg->fifo_r_w, read_size, data))
						return -1;
		ind = 0;
		for (ii = 0; ii < packet_count; ii++) {
			short accel_cur[3], gyro_cur[3];
		...
		unsigned char *data)
{
	...
	if (mpu_reg_write(st.reg->bank_sel, 2, tmp))
		return -1;
	if (mpu_reg_write(st.reg->mem_r_w, length, data))
		return -1;
	return 0;
}
		...
		unsigned char *data)
{
	...
	if (mpu_reg_write(st.reg->bank_sel, 2, tmp))
		return -1;
	if (mpu_reg_read(st.reg->mem_r_w, length, data))
		return -1;
	return 0;
}
...
#define LOAD_CHUNK  (16)
	...
	if (mpu_reg_write(st.reg->prgm_start_h, 2, tmp))
		return -1;
...
static int setup_compass(void)
{
	...
	if (mpu_reg_write(st.reg->s0_addr, 1, data))
		return -1;

	/* Compass reads start at this register. */
	data[0] = AKM_REG_ST1;
	if (mpu_reg_write(st.reg->s0_reg, 1, data))
		return -1;

	/* Enable slave 0, 8-byte reads. */
	data[0] = BIT_SLAVE_EN | 8;
	if (mpu_reg_write(st.reg->s0_ctrl, 1, data))
		return -1;

	/* Slave 1 changes AKM measurement mode. */
	data[0] = st.chip_cfg.compass_addr;
	if (mpu_reg_write(st.reg->s1_addr, 1, data))
		return -1;

	/* AKM measurement mode register. */
	data[0] = AKM_REG_CNTL;
	if (mpu_reg_write(st.reg->s1_reg, 1, data))
		return -1;

	/* Enable slave 1, 1-byte writes. */
	data[0] = BIT_SLAVE_EN | 1;
	if (mpu_reg_write(st.reg->s1_ctrl, 1, data))
		return -1;
	...
	data[0] = 0x03;
	if (mpu_reg_write(st.reg->i2c_delay_ctrl, 1, data))
		return -1;

#ifdef MPU9150
	/* For the MPU9150, the auxiliary I2C bus needs to be set to VDD. */
	data[0] = BIT_I2C_MST_VDDIO;
	if (mpu_reg_write(st.reg->yg_offs_tc, 1, data))
		return -1;
#endif
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
	...
	if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
		return -1;
#else
	if (mpu_reg_read(st.reg->raw_compass, 8, tmp))
		return -1;
#endif
	...
	unsigned short lpa_freq)
{
	...
	if (lpa_freq) {
		...
		data[0] |= BIT_I2C_IF_DIS;
		#endif
		if (mpu_reg_write(st.reg->user_ctrl, 3, data))
			goto lp_int_restore;

		/* Set motion threshold. */
		data[0] = thresh_hw;
		if (mpu_reg_write(st.reg->motion_thr, 1, data))
			goto lp_int_restore;
		...
		if (mpu_reg_write(st.reg->lp_accel_odr, 1, data))
			goto lp_int_restore;

		/* Enable motion interrupt (MPU6500 version). */
		data[0] = BITS_WOM_EN;
		if (mpu_reg_write(st.reg->accel_intel, 1, data))
			goto lp_int_restore;
